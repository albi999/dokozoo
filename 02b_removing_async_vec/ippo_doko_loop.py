...
num_envs = 1
env = turn_based_aec_to_parallel(tb_doko_env_V3.env())       
env.reset()
...
while agent.steps[-1] < max_steps:
    ...
    # print(f"obs: {obs}")
    scores = np.zeros(len(agent.agent_ids))
    ...
    for _ in range(agent.learn_step):
        ...
        for _ in range(40):
            ...
            print(f"action_items: {action.values()}") """



            # changed: Clipping to action space in my case not necessary
            ...
            # changed for removing async_vec
            agent_rewards = np.array(list(reward.values()))
            agent_rewards = np.where(np.isnan(agent_rewards), 0, agent_rewards)
            score_increment = np.sum(agent_rewards)
            ...
            for agent_id in termination:
                ...
                next_done[agent_id] = bool(terminated or truncated)
