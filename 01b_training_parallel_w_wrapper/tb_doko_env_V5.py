...
class raw_env(AECEnv):
    ...
    def __init__(self, render_mode: Union[str, None] = None):
        ...
        self.action_spaces = {agent_name: Discrete(21) for agent_name in self.possible_agents} # V4
    ...
    def reset(self, seed=None, options=None):
        ...
        for agent in self.agents:
            if agent == self.agent_selection:
                self.infos[agent] = {'action_mask': self.action_mask_calc()}
            else:
                self.infos[agent] = {'action_mask': np.zeros(21, dtype=int)} # V4
    ...
    def step(self, action):
        ...
        if self._agent_selector.is_last():
            ...
            if self.teams[trick_winner-1]:
                ...
                self.rewards[f"agentt{trick_winner}"] += trick_points
                self._cumulative_rewards[f"agentt{trick_winner}"] += trick_points

                trick_winning_indices = np.where(self.teams == 1)[0]
                for agent_index in trick_winning_indices:
                    agent_number = agent_index+1
                    if agent_number != trick_winner:
                        # aka the Partner
                        # V5 I want to give the partner a reward to how many points he gave to the trick
                        agent_string = "agentt" + str(agent_number)
                        self.rewards[agent_string] += self.determine_partner_reward(agent_number)
                        self._cumulative_rewards[agent_string] += self.determine_partner_reward(agent_number)
            ...
            if r == 9:
                ...
                if self.render_mode == "ansi":
                    for name in self.agents:
                        print(f"{name} | cumulative reward {self._cumulative_rewards[name]}")
    ...
    def determine_partner_reward(self, partner_number):
        # getting round, trick starter
        r = self.round
        trick_starter = self.starter if r == 0 else self.tricks_won_by[r-1] # P1-4
        trick = self.cards_played[r]

        # get card played by partner
        index = (partner_number-trick_starter)%4
        played_card_number = self.cards_played[r][index]
        card = self.unique_cards[played_card_number-1]
        if card.rank == 'A':
            return 11
        elif card.rank == '10':
            return 10
