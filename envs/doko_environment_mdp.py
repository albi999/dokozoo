...
class raw_env(AECEnv):
    ...
    def step(self, action):
        ...
        if round==0:
            # possible health calls: 0 (healthy), 1 (Wedding), 2 (Poverty)
            self.health_calls[agent_number-1] = action+1

            if self._agent_selector.is_last():
                # TODO game_type is determined
                gt = calc_game_type()
                self.game_type = gt

                # normalspiel
                if gt == 1: 
                    # TODO self.teams is defined
                ...
                #wedding
                    ...
                    # TODO jump to round 2 
                    # after first trick self.teams is defined
                #poverty
                elif gt == 4: 
                    # TODO jumpt to round 1 to see if there are any takers for the poverty
                
                else:
                    print("Something went wrong and your code fucking sucks")
                    print("@step() not a valid game type")

        if round == 1:
            # extra round for poverty taking
            # actions are either 0 (no) or 1 (yes)
            if action == 1: # TODO and not determined 
                # TODO self.teams is determined
            if self._agent_selector.is_last():
                # TODO if self.teams not determined --> new hand 
                # else 
    ...
    def observation_space(self):
        observation_space = SpaceDict({
            ...
            "round": Discrete(10),
            "health_calls": MultiDiscrete(4 * [4]),
            "game_type": Discrete(5),
