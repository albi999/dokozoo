""" Originally MATD3 Tutorial for speaker listener by
Authors: Michael (https://github.com/mikepratt1), Nickua (https://github.com/nicku-a)

Changing to IPPO Tutorial for Simple Spread Environment
"""
...
import numpy as np
import torch
from pettingzoo.mpe import simple_spread_v3 # NEW different environment
from tqdm import trange

from agilerl.algorithms import IPPO # NEW IPPO algorithm
...
if __name__ == "__main__":
    ...
    env = AsyncPettingZooVecEnv(
        [
            lambda: simple_spread_v3.parallel_env(continuous_actions=True)
    ...
    pop: List[IPPO] = create_population(
        INIT_HP["ALGO"],
        observation_spaces,
        action_spaces,
        NET_CONFIG,
    ...
    while np.less([agent.steps[-1] for agent in pop], max_steps).all():
        ...
        for agent in pop:  # Loop through population
            ...
            for idx_step in range(evo_steps // num_envs):
                ...
                actions, action_logprobs, dist_entropys, state_values = agent.get_action(
                    obs=obs, infos=info
                )
                action = actions
